...
import 'mocha'
import { PlayerStatsEvent } from '../src/PlayerStatsEvent'
...
describe('stats/PlayerStatsEvent', function() {
  it('should create an object out of the QL event', function() {
    ...
    expect(event.team).to.equal('Blue')
    ...
    expect(event.weapons.proximityLauncher.deaths).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['D'])
    expect(event.weapons.proximityLauncher.damageGiven).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['DG'])
    expect(event.weapons.proximityLauncher.damageReceived).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['DR'])
    expect(event.weapons.proximityLauncher.hits).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['H'])
    expect(event.weapons.proximityLauncher.kills).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['K'])
    expect(event.weapons.proximityLauncher.p).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['P'])
    expect(event.weapons.proximityLauncher.shots).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['S'])
    expect(event.weapons.proximityLauncher.t).to.equal(ql['DATA']['WEAPONS']['PROXMINE']['T'])
